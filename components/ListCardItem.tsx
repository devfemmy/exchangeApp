...
const ListCardItem = ({ data, id, handlePress, icon,modeInfo, isSwitchOn, onToggleSwitch }: any) => {
    ...
    <Pressable onPress={data?.name !== 'Allow Biometrics' ? handlePress : null}>
        <View style={styles.rowBtw}>
            <View style={styles.row}>
                <View>{icon}</View>
                <Text style={{...FONTS.body3, marginLeft: hp(15), color: modeInfo ? COLORS.black : data?.route === "DeleteAccount" ? "red" : COLORS.white}}>{data?.name}</Text>
            </View>
            <View>
            {
