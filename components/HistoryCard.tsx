...
import { capitalizeWord, format, hp } from '../utils/helper';
  ...
  return (
   <TouchableOpacity onPress={() => handleClick(data)}>
     <View style={[GlobalStyle.rowBetween, styles.card]}>
      <View style={[GlobalStyle.rowStart, {width: "70%"}]}>
        ...
        <View>
            <Text>
            <Text style={{textTransform: 'capitalize', ...FONTS.body4, color: modeInfo ? COLORS.lightBlack : COLORS.white}}>{capitalizeWord(transactionType)}</Text>
      ...
      <View style={{width: "28%",}}>
            <Text  style={{fontWeight: '600', textAlign: 'right', ...FONTS.body4,color: modeInfo ? COLORS.lightBlack : COLORS.white}}>{amount % 1 === 0 ? format(parseFloat(amount)) : amount.toString().match(/^-?\d+(?:\.\d{0,4})?/)[0]} {currency}</Text>
